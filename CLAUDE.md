...
- `src/` (React) : `features/{flightdeck,conversation,editor,terminal,git,explorer,settings}`, `ipc/`, `store/`, `agent/`, `notifications/`, `ui/`
  ...
  - `notifications/` : `notify.ts`, `sound.ts` (carillon Web Audio, zéro asset — ⚠️ **contexte FRAIS par lecture** : ne JAMAIS garder un `AudioContext` long-lived. WKWebView le rend **muet EN SILENCE** (`state` reste `"running"`, `resume()` ne répare pas, aucun signal JS pour « running mais muet ») dès qu'une autre app / une `<video>` reconfigure l'unité audio macOS → chaque `playChime` **crée puis `close()`** son propre contexte ; plafond `MAX_LIVE` 4 (limite WebKit) ; « Tester le son » crée le sien DANS le geste de clic. `closeContext` tolère `close()`→`undefined` (ancien WebKit). Vaut pour toute future feature Web Audio/média.), `transition.ts` (`agentEventFor` — point UNIQUE de détection des transitions d'état agent).
  - `src/ui/` : `Toggle.tsx`, `kit.tsx` (`ContextMeter`, `TodoPips`, `ChipBtn`, `Dot` avec prop `ring`), `ConfirmDialog.tsx` (modale oui/non réutilisable, props `danger`/`busy`, portal), `shortcuts.ts` (helpers purs robustesse AZERTY + registre `ACTION_BINDINGS`/`matchChord` + catalogue d'affichage `SHORTCUT_GROUPS`).

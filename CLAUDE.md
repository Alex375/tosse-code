...
- `src-tauri/` (Rust) : `supervisor/`, `git/`, `fs/`, `usage/`, `terminal/`, `store/`, `tosse/`, `ipc/`
  ...
  - `fs/` : `src-tauri/src/fs/mod.rs` = SEUL service filesystem de l'éditeur. `read_dir`, `read_file` (garde binaire/trop-gros >2 Mio), `write_file`, `read_image` (base64 des octets, garde `MAX_FILE_BYTES` 16 Mio — réutilisé par le viewer ET l'attachement d'images du composeur), `FsWatcher` (notify, debounce ~150 ms, filtre `.git/node_modules/target/dist/build/…`).
...
- **Durée des tours** (pref `showTurnDuration`, défaut ON, `tosse:display` — **UN seul réglage gate DEUX surfaces**, Réglages → Général → Affichage) : (1) **pied de tour statique** `TurnResultRow` (`ConductorThread.tsx`) affichant `result.duration_ms` (wall-clock du tour COMPLET : réflexion + appels modèle + tous les outils + réponse) sous chaque tour terminé — le composant ne rendait AVANT que les erreurs, la branche erreur est préservée ; format via `fmtDuration` ; SANS prix (écarté). (2) **compteur LIVE** dans `WorkingIndicator` au-delà de 40 s (`TURN_ELAPSED_MIN_MS`), leaf isolé `LiveElapsed` + hook local `useNow(1000)`, secondes floorées (affichage stable), façon CLI. Mécanique live = champ `turnStartedAt: number|null` sur `SessionEntry`, stampé dans `store/conversationStore.ts` `applyState` sur l'edge `busy` false→true (**EDGE-gaté** : jamais reset sur les re-emit `busy:true` mid-tour, sinon le compteur repartirait de 0), remis à `null` sur true→false + `clearState`. **Posé dans la couche STORE** (source de vérité de `busy`), PAS dans `useGlobalSessionEvents` (réservé à la détection de transition pour NOTIFICATIONS). `duration_ms` déjà propagé jusqu'à `TurnResultMeta.durationMs` → **100% front, zéro Rust**. Un tour rejoué depuis l'historique on-disk n'a PAS de `turn_result` → ni footer ni live à la relecture (attendu). ⚠️ Dette pré-existante NON corrigée : `fmtDuration` (`agent/subagentMeta.ts`) peut rendre « Xm 60s » (`Math.round(s%60)` peut valoir 60 sans retenue), partagé par ~6 call-sites (AgentBar, WorkflowDetail, SpecialMessageCard…) ; le compteur live y échappe (secondes floorées). [mergé dev — commit `c330d49`]

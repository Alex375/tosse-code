...
- **Durées & temps affichés** (groupe Réglages → Général → « Durées & temps » : **4 prefs `tosse:display` indépendantes, toutes défaut ON**, chacune gate SON affichage) :
  ...
  - **`showModelTime`** — le « · N s de modèle » du pied de tour (`result.duration_api_ms`). Capté côté Rust (avec `ttft_ms`, non affiché) : `ResultMsg`→`ConversationItem::TurnResult`→`ingest_result`→bindings régénérés→`TurnResultMeta`. Rider du footer (visible seulement si `showTurnDuration` ON).

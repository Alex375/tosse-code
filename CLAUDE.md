...
- `src-tauri/` (Rust) : `supervisor/`, `git/`, `fs/`, `usage/`, `terminal/`, `store/`, `tosse/`, `ipc/`
  ...
  - `usage/` : `src-tauri/src/usage/mod.rs` = SEUL service credentials OAuth + endpoint d'usage. `GET https://api.anthropic.com/api/oauth/usage` (% forfait 5h/7j — depuis 2026-07 l'endpoint renvoie AUSSI un tableau `limits[]` où chaque fenêtre déclare `is_active` ; on n'affiche une fenêtre que si sa limite est active — `session`→5h, `weekly_all`/`weekly`→7d — sinon l'objet `five_hour` obsolète, encore émis alors qu'Anthropic retire la limite 5h/session, s'afficherait à ~0 %). Token depuis `~/.claude/.credentials.json` → Keychain macOS (`/usr/bin/security`). **Lecture seule** (jamais de refresh ni d'écriture de credentials).

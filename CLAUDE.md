...
- **Menu slash-commands `/` du composer LIVRÉ** : catalogue issu du `initialize` control_response (cf. mémoire `slash-commands-from-initialize-response`), caché par **cwd** dans `src/store/commandsStore.ts` (localStorage `tosse:slash-commands-by-cwd`) → marche avant le spawn paresseux. `SlashCommandMenu.tsx` (helpers PURS testés) groupe par **scope dérivé côté front** des hints que la CLI encode dans la description (AUCUN champ scope sur le fil — vérité terrain 2.1.187) : `commandScope()` → **Projet** (`.claude/skills` du repo, `(project)` en queue) en tête, puis **Commandes** (built-ins + globaux utilisateur, indistinguables sans hint), puis une section par **plugin** A→Z (`(nom)` de tête / nom `plugin:cmd`). `deep-research` & co. `(dynamic workflow)` = général. `cleanDescription` retire les marqueurs `(project)`/`(dynamic workflow)` de queue sans toucher aux parenthèses légitimes (ex. `clear` → « resumable with /resume »). `commandScope` est **réutilisable** par le viewer extensions-par-scope (tâche 2ab34b91). **Rafraîchi après `/reload-skills`** : `refetchSlashCommands(cwd)` force un re-fetch qui **écrase** le cache (spawn `claude` éphémère relit le disque à l'`initialize`), détecté au point unique `sendText` de `ConductorComposer.tsx` via `isReloadSkillsCommand` ; **invariant** échec/vide → cache préservé (jamais de menu vidé). Tests : `SlashCommandMenu.test.ts` + `commandsStore.test.ts`. **Zéro changement Rust/IPC** (le fil livre déjà tout ; scope dérivé côté front).
...
## Rendu Markdown — 3 modes (Classic / Warm / Minimal)
Réglage GLOBAL `markdownMode` (`src/store/display.ts`, persisté localStorage `tosse:display`, **défaut `warm`**), appliqué PARTOUT où du Markdown est rendu (thread, sous-agents, aperçu `.md`) car tout passe par `StreamMarkdown`. Mécanique : `StreamMarkdown` pose une classe stable `md-body` + `data-md-mode={mode}` sur sa racine → variantes en CSS dans `src/ui/conductor-markdown-modes.css` (keyé `.md-body[data-md-mode="warm"|"minimal"]`, attribut+classe → out-specifie les règles `.root` du CSS module ; **Classic = `src/features/conversation/Markdown.module.css` INTACT**, look GitHub assumé). Contexte React `src/features/conversation/markdownMode.ts` (propage le mode à `CodeBlock` + un flag « demo » à `FileMention`).
- **Coloration syntaxique** : dépendance `highlight.js` (11.x) importée en **LAZY** (`import('highlight.js/lib/common')`, chunk séparé hors bundle de démarrage), warm/minimal uniquement, **langages taggés ET connus seulement** (pas de `highlightAuto` — perf + zéro fausse détection) ; couleurs de tokens par mode dans le CSS, **aucun thème hljs importé** ; HTML surligné clé sur le `code` courant (pas de contenu périmé au streaming). Classic reste brut.
- **Chip de chemin de fichier** (`src/features/conversation/FileMention.tsx` + helper pur testé `pathSegments.ts`) : rendu segmenté (icône fichier/dossier selon extension, dossiers atténués, nom saillant, `:ligne` coral) **réservé aux VRAIS fichiers** (gating `target != null || demo`) — un slash non-fichier (nom de skill…) reste du code inline normal, sans icône. Classic = segmenté mais uniforme, sans icône.
- **Tableaux** : `StreamMarkdown` enveloppe chaque table dans `.md-tablewrap` ; warm/minimal = cadre arrondi + séparateurs internes, `overflow:auto` (scroll horizontal préservé). Classic garde la grille GitHub.
- **Réglages → section « Conversation »** (`src/features/settings/ConversationSection.tsx`) : rail 3 modes + grand aperçu live (StreamMarkdown mode forcé + `demoFilePaths`, inerte au clavier). Panneau Réglages en taille FIXE agrandie (1040×700) pour tous les onglets.
